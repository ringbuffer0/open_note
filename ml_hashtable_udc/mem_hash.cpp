...
void MemHash::MemInitNew()
{
    ...
    printf("bucket_time=%d,bucket_len=%d,max_block=%d\n",head_->head_info_.bucket_time,head_->head_info_.bucket_len, head_->head_info_.max_block );
	//计算crc32用于下次使用时效验
    printf("MemInitNew\n");
    printf("str=%s,len=%d\n",( char *)&(head_->head_info_),sizeof(head_->head_info_));
	head_->crc32_head_info = Crc32Compute((char  *)&(head_->head_info_), sizeof(head_->head_info_));
...
void MemHash::CheckHead()
{
	...
	if (crc32_check != head_->crc32_head_info) {
		printf("MemHash::CheckHead  error.%0x,%0x\n",crc32_check, head_->crc32_head_info); 
		exit(-1);
	}
}
...
void MemHash::Stat(uint32_t& node_used, uint32_t& block_used)
{
    ...
    printf("%d,%d\n",head_->node_used,head_->block_used );
}
...
uint32_t MemHash::Crc32Compute(const char* oridata, int len)
{
    /*
    int j=0;
    printf("str=%s,%d\n",oridata,len);
...
*/
	...
	uint32_t reg = 0;
	for(int i = 0; i < len; i++) {
        int index=(data[i] ^ ((reg >> 24)) & 0xFF);
        if(index>=256){
            printf("error to find a invalid index,%d\n",index);
        }
        else{
            ///index=ffffff01 core dump
            printf("index=%0x\n",(data[i] ^ ((reg >> 24)) & 0xFF));
            printf("data[i]=%0x,reg=%0x\n",data[i],reg);
            //data[i]=0x0;
        }
		reg = (reg << 8) ^ crc32_table[(data[i] ^ ((reg >> 24)) & 0xFF)];
        printf("reg=%0x,%0x\n",reg,(data[i] ^ ((reg >> 24)) & 0xFF));
	}       
	*/
